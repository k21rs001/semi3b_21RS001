...
## 3.設計書
　本章では、本研究の概要について詳しく述べる。まず、3.1節では本研究の開発環境と使用するPythonライブラリについて紹介する。次に、3.2節で研究の流れについて説明する。次に3.3節では進捗状況について説明する。最後に、3.3節では設計書に関する今後の展望について説明する。
### 3.1 開発環境
使用パソコン：九州産業大学貸与PC  
...
　このプログラムの実行手順としては、`if __name__ == "__main__":`ブロックの中の読み込むフォルダのパスと出力フォルダのパスを指定し、実行するだけである。実行後、出力フォルダにテキストファイルが保存される。

#### ③形態素解析を行なう。
　抽出したテキストファイルから形態素解析を行なう。ライブラリはMeCabを使用する。品詞ごと抽出し、品詞ごとの頻出語をカウントする。
